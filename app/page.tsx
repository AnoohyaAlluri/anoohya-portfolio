...
Anoohya_Alluri_Data_Analyst_3(1).docx
Document
DO i r accrding to this portfolio 


Anoohya_Alluri_Data_Analyst_3(1).docx
Document
DO i r accrding to this resume

this is where i am and did i stop any rect in  middle or do  i ened to ad dany orejct t teh ecurrent poretfolio 

on;y few have got hinb links and tablsue wht aabout teh rest >?? nwo accodu rf got my git hub and proetfiloi help me 
...
Pasted text(11).txt
Document
Updat etshi swhiel code an dgiv eme ?????



Pasted text(12).txt
Document
 Done Updated with this >

run_pipeline.py  i cnat firnd it 

i dont wna review genertn here my prejet s section is not storng 


Pasted text(13).txt
Document
 did i relace it correclty ?? 

  i dnt wna thsis it isnt strong in git hub 

  this iamg is notstrong a dn tfeeional eough see all te lines 

Worked for 49s
Generated image: Internal linking strategy flowchart
...
Pasted text(14).txt
Document
    cat find you uod ate the howle thisng ad tell me 

.,   I deleted cirrent reume fiel 


MDA.pdf
PDF
i WNAT TO UPDAD THSI STEHRE IS FLDER Holde rin public named reume wher ei cn aupald tshis file 

Today 10:34 AM
a8a1a3d7-b315-4340-b2dc-8391b2be3023.png
  is this correct 

f3d5a49c-61b6-40b2-9c4c-249192585dde.png

78a22103-7b72-4508-82b7-0dcf5d43cc0f.png
724cefa6-f121-45fc-9ecb-efc15617213b.png
  Tere is an error but i vncat open it ??? 

Worked for 45s
Correct. You want the resume filename to change, while the portfolio button continues working automatically.

The clean solution is to make the portfolio button open a permanent route:

/resume
That route will locate whichever single PDF currently exists inside:

public/Resume/
Next.js Route Handlers can redirect users programmatically, while files in public are served from the website root. 
...
export function GET() {
  const resumeFolder = path.join(process.cwd(), "public", "Resume");

  const resumeFiles = readdirSync(resumeFolder).filter((file) =>
  ...
  if (resumeFiles.length > 1) {
    return new Response(
      "Multiple resume PDFs found. Keep only one PDF inside public/Resume.",
      {
        status: 500,
      }
    );
  ...
  redirect(`/Resume/${encodeURIComponent(resumeFiles[0])}`);
}
Commit with:

Create automatic resume redirect
Keep your current file named:

MDA.pdf
Do not rename it. After this route is added, we will change the portfolio button to /resume.



Pasted text(16).txt
Document
     Hleo me iup date this accrodingly 

Worked for 25s
I updated the complete portfolio code you uploaded so the View Resume button now opens:

/resume
instead of a specific PDF filename. 


The separate route file automatically finds the single PDF inside public/Resume and redirects to it. Next.js supports custom route.ts handlers inside the app directory, while files inside public are served from the site’s root path. 

Step 1: Replace app/page.tsx
Download the updated complete file:

anoohya-portfolio-page-resume-route.tsx
...
Replace everything with the downloaded code

Commit with:

Connect portfolio to automatic resume route
Step 2: Create the automatic resume route
Download:
...
resume/route.ts
Paste the complete contents of the downloaded resume-route.ts

Commit with:

Create automatic resume redirect
Required folder rule
Keep exactly one PDF inside:

public/Resume/
Your current file can remain:

MDA.pdf
In the future, the replacement can have any filename, for example:

Marketing-Data-Analyst-Resume.pdf
Anoohya-Alluri-Resume-2026.pdf
Senior-Marketing-Analytics-Resume.pdf
Delete the previous PDF before uploading the replacement. The portfolio code will not need to be changed again.

After Vercel deploys, test:

https://anoohya-portfolio.vercel.app/resume



did you gi vm eteh ewhoel pudated oen ?? 

Yes. I gave you the complete updated app/page.tsx file, not only the changed section. It contains all the code you pasted, with this one resume-link update:

href="/Resume/MA.pdf"
changed to:

href="/resume"
Your full original portfolio structure remains included. 


Complete files
Full updated portfolio page
...
Separate automatic resume route
...
app/resume/route.ts
Both files are required. The first updates the button; the second automatically locates whichever single PDF is inside public/Resume.
